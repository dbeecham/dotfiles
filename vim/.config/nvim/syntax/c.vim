...
if exists("c_curly_error")
  ...
  "syn region	cBlock		start="{" end="}" contains=TOP,cBadBlock,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell fold
  syn region	cBlock		start="{" end="}" contains=TOP,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell fold
else
...
if exists("c_no_curly_error")
  if s:ft ==# 'cpp' && !exists("cpp_no_cpp11")
    syn region	cParen		transparent start='(' end=')' contains=TOP,@cParenGroup,cCppParen,@cStringGroup,@Spell
    " cCppParen: same as cParen but ends at end-of-line; used in cDefine
    syn region	cCppParen	transparent start='(' skip='\\$' excludenl end=')' end='$' contained contains=TOP,@cParenGroup,cParen,cString,@Spell
    syn match	cParenError	display ")"
    syn match	cErrInParen	display contained "^^<%\|^%>"
  else
    syn region	cParen		transparent start='(' end=')' end='}'me=s-1 contains=TOP,cBlock,@cParenGroup,cCppParen,@cStringGroup,@Spell
    " cCppParen: same as cParen but ends at end-of-line; used in cDefine
    syn region	cCppParen	transparent start='(' skip='\\$' excludenl end=')' end='$' contained contains=TOP,@cParenGroup,cParen,cString,@Spell
    syn match	cParenError	display ")"
    syn match	cErrInParen	display contained "^[{}]\|^<%\|^%>"
...
elseif exists("c_no_bracket_error")
  ...
  if s:ft ==# 'cpp' && !exists("cpp_no_cpp11")
    syn region	cParen		transparent start='(' end=')' contains=TOP,@cParenGroup,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell
    " cCppParen: same as cParen but ends at end-of-line; used in cDefine
    syn region	cCppParen	transparent start='(' skip='\\$' excludenl end=')' end='$' contained contains=TOP,@cParenGroup,cErrInBracket,cParen,cBracket,cString,@Spell
    syn match	cParenError	display "[\])]"
    syn match	cErrInParen	display contained "<%\|%>"
    syn region	cBracket	transparent start='\[\|<::\@!' end=']\|:>' contains=TOP,@cParenGroup,cErrInParen,cCppParen,cCppBracket,@cStringGroup,@Spell
  else
    syn region	cParen		transparent start='(' end=')' end='}'me=s-1 contains=TOP,cBlock,@cParenGroup,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell
    " cCppParen: same as cParen but ends at end-of-line; used in cDefine
    syn region	cCppParen	transparent start='(' skip='\\$' excludenl end=')' end='$' contained contains=TOP,@cParenGroup,cErrInBracket,cParen,cBracket,cString,@Spell
    syn match	cParenError	display "[\])]"
    syn match	cErrInParen	display contained "[\]{}]\|<%\|%>"
    syn region	cBracket	transparent start='\[\|<::\@!' end=']\|:>' end='}'me=s-1 contains=TOP,cBlock,@cParenGroup,cErrInParen,cCppParen,cCppBracket,@cStringGroup,@Spell
  endif
...
if !exists("c_no_ansi") || exists("c_ansi_constants") || exists("c_gnu")
  ...
  syn keyword cConstant AF_UNSPEC SOCK_STREAM
  syn keyword cConstant ret
  " POSIX 2001
...
hi def link cIncluded		cInclude
...
syntax match cNiceOperator "||" conceal cchar=∨
syntax match cNiceOperator "->" conceal cchar=→
syntax match cNiceOperator "_len" conceal cchar=#

"syntax match cSizeOfEnd "[^)]*" contained nextgroup=cSizeOfEnda
"syntax match cSizeOfEnda ")" conceal contained cchar=ǁ
"syntax match cSizeOfBegin "sizeof(" contained conceal cchar=ǁ nextgroup=cSizeOfEnd
"syntax match cSizeOf "\vsizeof" contains=cSizeOfBegin

"hi link cSizeOf Error

"syntax match cSizeOfInside "[a-zA-Z _]*" contained conceal cchar=C nextgroup=cSizeOfEnd
" syntax match cSizeOfInside "[a-zA-Z _]*"
"syntax region cSizeOfRegion matchgroup=cSizeOfInside start="sizeof(" end=")" concealends cchar=|
